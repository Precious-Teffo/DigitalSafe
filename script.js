...
function updateStats() {
    document.getElementById('threatsBlocked').textContent = userData.threatsAnalyzed;
...
// Educational content library - 5 MOST IMPORTANT LESSONS
const lessons = {
    passwords: {
        ...
        content: `
            ...
            <div class="lesson-section">
                ...
                <ul style="margin-left: 20px; line-height: 1.8; color: #475569;">
                    ...
                    <li><strong>Torrenting:</strong> Hide activity from ISP (legal content only)</li>
                </ul>
            ...
            <div class="lesson-section">
                ...
                <p><strong>NordVPN ($3-12/month)</strong></p>
                <ul style="margin-left: 20px; line-height: 1.8; color: #475569;">
                    <li>✅ Fast servers worldwide</li>
                    <li>✅ Double VPN option</li>
            ...
            <div class="lesson-section">
                <h2>VPN Features to Look For</h2>
                
                <ul style="margin-left: 20px; line-height: 1.8; color: #475569;">
                    <li><strong>No-logs policy:</strong> VPN shouldn't record your activity</li>
                    <li><strong>Kill switch:</strong> Blocks internet if VPN disconnects</li>
                    <li><strong>Strong encryption:</strong> AES-256 minimum</li>
                    <li><strong>DNS leak protection:</strong> Prevents DNS requests from bypassing VPN</li>
                    <li><strong>Multiple protocols:</strong> OpenVPN, WireGuard, IKEv2</li>
                    <li><strong>Server locations:</strong> More locations = better performance</li>
                </ul>
            </div>
...
function openLesson(lessonId) {
    ...
    content.innerHTML = `
        ...
        <h1 style="color: #1e293b; font-size: 2.5em; margin-bottom: 32px; text-align: center;">${lesson.title}</h1>
        ${lesson.content}
    ...
    try {
        const response = await fetch("https://api.anthropic.com/v1/messages", {
            ...
            body: JSON.stringify({
                ...
                messages: [{
                    role: "user",
                    content: `Analyze for threats and respond with JSON:
{
...
Message: "${input}"`
        ...
        if (analysis) {
            ...
            resultDiv.innerHTML = `
                <div style="background: white; border: 2px solid ${colors[analysis.threatLevel]}; border-radius: 12px; padding: 24px; margin-top: 20px;">
                    <div style="font-size: 1.5em; font-weight: 700; color: ${colors[analysis.threatLevel]}; margin-bottom: 16px;">
                        ${analysis.threatLevel} Threat - ${analysis.confidence}% Confidence
                    </div>
                    <div style="margin-bottom: 16px;">
                        <strong>Analysis:</strong>
                        <p style="margin-top: 8px;">${analysis.explanation}</p>
                    </div>
                    <div style="margin-bottom: 16px;">
                    ...
                    <div style="background: #f0f9ff; padding: 16px; border-radius: 8px;">
                        ...
                        <p style="margin-top: 8px;">${analysis.recommendation}</p>
                    </div>
                </div>
            `;
            
            userData.threatsAnalyzed++;
            updateShieldScore(25, 'Threat Analyzed');
        }

    } catch (error) {
        console.error('Error:', error);
        resultDiv.innerHTML = `
            <div style="background: #fee2e2; border: 2px solid #ef4444; border-radius: 12px; padding: 24px; margin-top: 20px;">
                <strong>Analysis unavailable</strong>
                <p style="margin-top: 8px;">Please try again.</p>
            </div>
        `;
    } finally {
        btn.disabled = false;
...
function checkDailyChallenge() {
    ...
    challengeDiv.innerHTML = `
        <div style="text-align: center; padding: 20px;">
            ...
            <div style="font-weight: 700;">Daily Challenge Loading...</div>
        </div>
    `;
}
...
function populateThreatFeed() {
    ...
    const threats = [
        { title: 'Amazon Phishing', desc: 'Fake suspension emails', level: 'critical', time: '12 min ago' },
        { title: 'IRS Tax Scam', desc: 'SMS about refunds', level: 'high', time: '1 hour ago' }
    ];
    ...
    feed.innerHTML = threats.map(t => `
        <div class="threat-alert ${t.level}">
            <div class="alert-header">
                ...
                <span class="alert-badge ${t.level}">${t.level}</span>
            </div>
            <div style="color: #9ca3af;">${t.desc}</div>
        </div>
    `).join('');
}
...
const socialEngScenarios = [
    ...
    },
    {
        ...
        role: "FedEx Delivery Services",
        message: "We attempted to deliver your package but no one was home. Click here to reschedule delivery and pay the $3.95 redelivery fee: fedex-redelivery.com",
        correctResponse: "suspicious"
    },
    {
        name: "Microsoft Security Warning",
        role: "Microsoft Technical Support",
        message: "Your computer is infected with a critical virus that's stealing your personal data. Download our security tool immediately or call 1-800-VIRUS-HELP for urgent assistance.",
        correctResponse: "suspicious"
    },
    {
        name: "IRS Tax Refund",
        role: "Internal Revenue Service",
        message: "You are eligible for a tax refund of $2,847.63. Click here to submit your banking information to receive your refund within 48 hours.",
        correctResponse: "suspicious"
    },
    {
        name: "LinkedIn Job Opportunity",
        role: "Senior Recruiter at Google",
        message: "I found your profile impressive! We have an urgent opening for your skillset with $200K salary. Please download and complete this application form ASAP: google-careers-app.exe",
        correctResponse: "suspicious"
    },
    {
        name: "Colleague Document Request",
        role: "Your Coworker Sarah",
        message: "Hey! I'm working from home today and can't access the shared drive. Can you open this file and email me the Q3 numbers? Thanks! [attachment: Q3_Report.zip]",
        correctResponse: "verify"
    },
    {
        name: "Charity Donation Appeal",
        role: "Disaster Relief Foundation",
        message: "Help victims of the recent earthquake! Donate now via wire transfer or gift cards. Every dollar counts. Act now before it's too late!",
        correctResponse: "suspicious"
    },
    {
        name: "Netflix Account Suspension",
        role: "Netflix Billing Department",
        message: "Your Netflix account has been suspended due to billing failure. Update your payment information within 24 hours to avoid permanent account deletion.",
        correctResponse: "suspicious"
    }
];
...
function respondToSocialEng(response) {
    ...
    if (isCorrect) {
        ...
        resultDiv.innerHTML = `<div style="background: #dcfce7; padding: 20px; border-radius: 12px;">Perfect Response! +50 Shield Score</div>`;
    } else {
        resultDiv.innerHTML = `<div style="background: #fee2e2; padding: 20px; border-radius: 12px;">You've been compromised!</div>`;
    }
...
function checkBreach() {
    ...
    resultDiv.innerHTML = `<div class="ai-insight">Checking...</div>`;
    resultDiv.style.display = 'block';

    setTimeout(() => {
        const hasBreaches = Math.random() > 0.5;
        if (hasBreaches) {
            resultDiv.innerHTML = `<div class="ai-insight" style="background: #fef2f2; border: 2px solid #fca5a5;">Breaches Found! Change passwords immediately.</div>`;
        } else {
            resultDiv.innerHTML = `<div class="ai-insight" style="background: #dcfce7; border: 2px solid #86efac;">No breaches found. Stay safe!</div>`;
        }
    }, 2000);
}
